<template>
  <div style="margin:10px 0px">
    ...
    <div style="margin-bottom: 20px">
      <!-- 添加与批量删除按钮 -->
      <el-button v-show="$store.state.user.buttons.indexOf('btn.User.add')!=-1" type="primary" @click="showAddUser">添 加</el-button>
      <el-button v-show="$store.state.user.buttons.indexOf('btn.User.remove')!=-1" type="danger" @click="revomveUsers" :disabled="selectedIds.length===0"
        >批量删除</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="{row}">
          <HintButton 
            v-show="$store.state.user.buttons.indexOf('btn.User.assgin')!=-1" 
            type="info" 
            size="mini" 
          ...
          <HintButton 
            v-show="$store.state.user.buttons.indexOf('btn.User.update')!=-1" 
            type="primary" 
            size="mini" 
            icon="el-icon-edit" 
            title="修改用户"
          ...
          <el-popconfirm :title="`确定删除 ${row.username} 吗?`" @onConfirm="removeUser(row.id)">
            <HintButton v-show="$store.state.user.buttons.indexOf('btn.User.remove')!=-1" style="margin-left:10px" slot="reference" type="danger" size="mini" icon="el-icon-delete" 
              title="删除用户"/>
          </el-popconfirm>
        </template>
      </el-table-column>
